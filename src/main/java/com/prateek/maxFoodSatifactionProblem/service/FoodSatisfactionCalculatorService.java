...
public class FoodSatisfactionCalculatorService {
	...
	public int maximumSatisfaction(String input) {
		...
		if (rowsInList.length != totalItemsOnMenu + 1) {
			throw new CustomAPIException("Invalid Input. TotalItemsMentioned in Line1 = " + totalItemsOnMenu
					+ " but recieved only " + (rowsInList.length - 1) + "items");
		}
		...
		for (int satisFactionElement = 0; satisFactionElement <= satisfactionList.size(); satisFactionElement++) {
			for (int increasingTimeUnit = 0; increasingTimeUnit <= totalTimeT; increasingTimeUnit++) {
				...
				if (increasingTimeUnit - time.get(satisFactionElement - 1) >= 0) {
					maxSatisFactionCalulatorArray[satisFactionElement][increasingTimeUnit] = Math.max(
							maxSatisFactionCalulatorArray[satisFactionElement - 1][increasingTimeUnit],
							maxSatisFactionCalulatorArray[satisFactionElement - 1][increasingTimeUnit
									- time.get(satisFactionElement - 1)]
									+ satisfactionList.get(satisFactionElement - 1));
				} else {
					maxSatisFactionCalulatorArray[satisFactionElement][increasingTimeUnit] = maxSatisFactionCalulatorArray[satisFactionElement
							- 1][increasingTimeUnit];
				}
			}
		}
